            ...
            for prod in data['prods']:
                ...
                if validToken:
                    if(int(price) > prod['price']):
                        return {'price': price, 'lowestPrice': prod['price']}
...
with open('search.csv', newline='') as csvfile:
    ...
    for prod in prods:
        if prods[prod]:
            print('https://ecshweb.pchome.com.tw/search/v3.3/?q={}'.format(prod))
